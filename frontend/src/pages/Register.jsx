...
export default function Register({ onSwitchToLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.detail || 'فشل إنشاء الحساب، يرجى المحاولة مرة أخرى');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen min-h-[100dvh] bg-slate-950 flex flex-col justify-center items-center p-3.5 sm:p-6 relative overflow-hidden select-none" dir="rtl">
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-8 shadow-2xl relative z-10">
        <div className="text-center mb-6 sm:mb-8">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-emerald-600 to-green-400 flex items-center justify-center mx-auto mb-3 sm:mb-4 shadow-xl shadow-emerald-950">
            <Zap className="w-6 h-6 text-white" />
          </div>
          <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">إنشاء حساب ريفيو فلو</h1>
          <p className="text-[11px] sm:text-xs text-slate-400 mt-1">ابدأ فترتك التجريبية وأتمت قنوات التداول فوراً</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pr-10 pl-4 py-3 rounded-xl bg-slate-950 border border-slate-800 focus:border-emerald-500 text-white text-sm outline-none"
                required
              />
            </div>
          </div>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">اسم القناة أو العلامة التجارية</label>
            <div className="relative">
              <Building className="w-4 h-4 text-slate-500 absolute right-3.5 top-3.5" />
              <input
                type="text"
                placeholder="قناة توصيات الذهب VIP"
                value={companyName}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:scale-98 text-white text-xs sm:text-sm font-bold shadow-lg shadow-emerald-950 transition-all flex items-center justify-center gap-2 mt-4"
          >
            <span>{loading ? 'جاري إنشاء الحساب...' : 'بدء الاستخدام الآن'}</span>
            <ArrowRight className="w-4 h-4 rotate-180" />
          </button>
        </form>

        <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-center text-xs">
          <button
            type="button"
