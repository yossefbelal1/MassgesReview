...
SESSION_STR = os.getenv("TELEGRAM_STRING_SESSION", "1BJWap1sBuypZntEY2gXJ8hZJ_zjFLJk7aNKxe9fGNB4bxM21iHnh9iNz5SaChPEr58S5SnQGd9vVwbJAN2-zmMfDPoKGoWy9RLI1MIl5_yCB3pfC747Qo4SYdR3sNPnry2errpIN9Jqv0W4oGHd0Q5g9z54AtdNsG2fYH0U2fvNzF9VrDm32L5FhbuL9aREY92gA8wFmXE-4jYR1KA3bMg2Byoylvreu7gGoKv1xDU7par5kKvb8-KT3RbryACir2R2MxpHZrQ_4CBMbBF4UL0pE0H26jIRn4QqFbWFXAXYRep0lBQ_575T2YgRSrOuUL0MWvButjTZxr2W88l5arf8jcFF3Kkk=")
...
async def dynamic_channel_listener(event):
    ...
    if match_tenant_trigger(tenant, msg_text):
        ...
        try:
            # سحب الرسائل من بنك الريفيوز (التي تحتوي على رسائل أعضاء حقيقيين)
            messages = await client.get_messages(bank_id, limit=50)
            valid_msgs = [m for m in messages if m.fwd_from and (m.message or m.media)]
            ...
            max_rev = tenant.get("max_reviews", 3)
            count = min(random.randint(min_rev, max_rev), len(valid_msgs))
            selected = random.sample(valid_msgs, k=count)

            delays = tenant.get("delay_range", [2.0, 4.0])

            print(f"[*] جاري إرسال {count} ريفيوز بأسماء أعضاء حقيقيين إلى '{tenant.get('name')}'...")
            for i, m in enumerate(selected, 1):
                ...
                fwd_name = getattr(m.fwd_from, 'from_name', 'عضو VIP') if m.fwd_from else 'عضو VIP'
                print(f" [✓] تم إرسال ريفيو {i}/{count} من العضو: '{fwd_name}'")
...
async def main():
    ...
    if not await client.is_user_authorized():
        print("[❌] Telegram Userbot is not authorized!")
        return
    me = await client.get_me()
    print("=" * 60)
    print(f"[🚀] Multi-Tenant Reviews Bot شغال وموثق كـ: {me.first_name} ({me.phone})")
